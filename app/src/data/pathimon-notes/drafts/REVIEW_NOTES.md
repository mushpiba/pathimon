...
## 2026-07-14 전투 타입/진화 설계 재검토
- 병원체 노트 전체에 `설계검토` 블록을 추가해 준비기/공격기/전용기 역할과 방어특성 방향을 표시했다.
- 전투 타입 큰 방향은 보스 공격 타입 = 인체 방어/치료 행위, 패시몬 방어특성 = 병원체 회피/생존 구조로 둔다.
- 준비기는 감염경로/정착과 랭크업/상태이상을 중심으로, 공격기는 실제 병인 공격 중심으로, 전용기는 생존/회피/생활사 특징 중심으로 검수한다.
- 공격기에 이미 붙은 상태이상은 독소/침습/조직손상 결과라면 유지 가능하고, 감염경로/정착 결과라면 준비기로 옮기는 후보로 본다.
- 기생충은 충란/유충/성충 단계별 노트와 기술 분리를 우선 검토하며, 진화의 돌 3원 가치가 보이도록 대부분 스탯/기술 상승을 둔다.
