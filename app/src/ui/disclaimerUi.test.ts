...
describe('disclaimer UI content', () => {
  it('shows a short non-commercial unfinished-project disclaimer before story', () => {
    ...
    expect(content.blinkEffect.cycles.map((cycle) => cycle.closeMs)).toEqual([200, 350, 500]);
    expect(content.blinkEffect.cycles.map((cycle) => cycle.closedMs)).toEqual([80, 200, 400]);
    expect(content.blinkEffect.cycles.map((cycle) => cycle.openMs)).toEqual([120, 180, 250]);
    expect(content.blinkEffect.cycles.map((cycle) => cycle.openHoldMs)).toEqual([800, 400, 150]);
    expect(content.blinkEffect.finalCloseMs).toBe(800);
    expect(content.blinkEffect.finalHoldMs).toBe(1200);
